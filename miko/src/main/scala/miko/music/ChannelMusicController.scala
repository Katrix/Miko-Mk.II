...
class ChannelMusicController(
  ...
  def sendMessage(
    ...
    textChannel(info.flatMap(_.tChannel)).foreach(chan => queue.offer(CreateMessage(chan.id, message)))
  ...
  override def onMessage(msg: Command): Behavior[Command] = msg match {
    ...
    case LavaplayerEvent(event: TrackStartEvent) =>
      sendServerEvent(MusicActivePlaying(guildId, event.track.getUserData.asInstanceOf[Long]))
    ...
    case ChannelMusicCommandWrapper(command, info) =>
      ...
      command match {
        ...
        case Pause =>
          ...
          sendServerEvent(MusicTogglePaused(guildId))
        ...
        case Volume(volume) =>
          ...
          sendServerEvent(MusicSetVolume(guildId, volume))
        ...
        case Clear =>
          sendServerEvent(MusicSetPlaylist(guildId, Nil))
          removeAllTracks()
        ...
        case Seek(progress, useOffset) =>
          ...
          sendServerEvent(MusicSeeked(guildId, progress.toInt))
        ...
        case ToggleLoop =>
          ...
          sendServerEvent(MusicToggleLoop(guildId))
  ...
  def stopMusic(): Unit = {
    sendServerEvent(MusicStopping(guildId))
    player.stopTrack()
  ...
  def addTrack(track: AudioTrack): Unit = {
    sendServerEvent(MusicQueued(guildId, musicEntryForTrack(track)))
  ...
  def addTracks(tracks: AudioTrack*): Unit = {
    ...
    sendServerEvent(MusicPlaylistQueued(guildId, tracks.tail.map(musicEntryForTrack)))
  ...
  def shuffleTracks(): Unit = {
    ...
    sendServerEvent(MusicSetPlaylist(guildId, res.map(musicEntryForTrack).toSeq))
